...
use sqlx_oldapi::{MssqlPool, query, Transaction};

use crate::{data_import::data_import_db::DbData, utils::{sql::client,error::MyError}};
...
pub async fn write_data_to_db(datas: DbData) -> anyhow::Result<()> {
    ...
    match tx.commit().await {
        Ok(_) => {
            info!("Successfully committed transaction. Total rows inserted: {}", rows_inserted);
            Ok(())
        },
        Err(e) => {
