        ...
        while let Some(item) = self.view.draw_walk(cx, scope, walk).step() {
            if let Some(mut list) = item.as_portal_list().borrow_mut() {
                if let Some(state) = scope.data.get::<Store>() {
                    ...
                    let keys_len = state.setting_store.map_infos.len();
        ...
        while let Some(item) = self.view.draw_walk(cx, scope, walk).step() {
            if let Some(mut list) = item.as_portal_list().borrow_mut() {
                if let Some(state) = scope.data.get_mut::<Store>() {
                    // info!("Drawing MapGrid with map_infos: {:?}", state.setting_store.map_infos);
                    let len = state.setting_store.map_infos.len().div_ceil(3);
