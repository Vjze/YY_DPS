...
use crate::{configs::{column_map_config::get_template_map_config, decimal_config::get_decimal_config_value, type_config::get_type_infos}, utils::error::MyError};

use super::{
    excel_utils::{
        before_im_num, before_ith_num, before_po, before_vf_num, calculate_tc,
...
pub async fn write_to_excel(
    ...
    if errors.is_empty() {
        ...
        Err(MyError::WriteErr("表格".to_string(),format!("{}",error)))
    }
}

fn format_with_decimals(val: &str, config: Option<&Value>) -> (String, Option<usize>) {
    match config {
        Some(Value::Number(n)) if val.parse::<f64>().is_ok() => {
            let num = val.parse::<f64>().unwrap();
            let decimals = n.as_u64().unwrap_or(2) as usize; // 获取配置的小数位数
...
fn calculate_row_value(
    ...
    // 规范化表头：转为小写，移除空格和括号
    let normalized_header = header.clone()
        .to_lowercase();
        // .replace(" ", "")
        // .replace("(", "")
        // .replace(")", "");
    ...
    } else if normalized_header.contains("po") {
        if normalized_header.contains("beforetc") {
            (
                (before_po(
                    row_data
